...
import {
  ...
  Clock, BriefcaseBusiness, Sparkles, Bell, Trash2,
} from "lucide-react";
  ...
  return (
    <div className="detail-panel">
      ...
      <div style={{ display:"flex", gap:10 }}>
        ...
        <button onClick={()=>onDelete(event.id)} style={{ flex:1, height:36, borderRadius:999, fontSize:12, fontWeight:600, background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.25)", color:"#ef4444", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:5, transition:"all 0.2s", fontFamily:"Montserrat, sans-serif" }}>
...
export default function CalendarPage() {
  const { user } = useAuth();
  const storageKey = `placetracker_events_${user?.email}`;
  const appsKey = `placetracker_apps_${user?.email}`;

  const today = new Date();
  const [year, setYear] = useState(today.getFullYear());
  const [month, setMonth] = useState(today.getMonth());
  const [events, setEvents] = useState(() => JSON.parse(localStorage.getItem(storageKey) || "[]"));
  ...
  const persist = (updated) => { setEvents(updated); localStorage.setItem(storageKey, JSON.stringify(updated)); };

  const handleSave = (form) => {
    if (editTarget) {
      persist(events.map(e => e.id === editTarget.id ? { ...form, id:e.id } : e));
      toast.success("Event updated!");
      setEditTarget(null);
    } else {
      persist([...events, { ...form, id: Date.now().toString() }]);
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#02050E", overflowX:"hidden", paddingTop:80, fontFamily:"Montserrat, sans-serif", position:"relative" }}>
      <style>{`
        ...
        .cal-grid-bg { position:fixed; inset:0; pointer-events:none; z-index:0; background-image:linear-gradient(rgba(143,160,215,0.035) 1px,transparent 1px),linear-gradient(90deg,rgba(143,160,215,0.035) 1px,transparent 1px); background-size:50px 50px; mask-image:linear-gradient(to bottom,black 0%,transparent 100%); }
        ...
        .month-title { fontFamily:"EB Garamond, serif"; font-size:22px; font-weight:600; color:#F8FAFF; }
        ...
        .cal-event-dot { font-size:10px; font-weight:600; padding:2px 7px; border-radius:4px; margin-bottom:2px; border:1px solid; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; cursor:pointer; transition:opacity 0.15s; display:block; }
                ...
                {Array.from({ length: totalCells }, (_, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {dayEvents.length > 3 && (
                        <span style={{ fontSize:10, color:"#5A6386", paddingLeft:2 }}>+{dayEvents.length-3} more</span>
                      )}
                    </div>
                  );
                })}
          ...
          <div className="cal-sidebar" style={{ width:300, flexShrink:0, display:"flex", flexDirection:"column", gap:16, opacity:mounted?1:0, transform:mounted?"none":"translateY(14px)", transition:"opacity 0.6s ease 0.18s, transform 0.6s ease 0.18s" }}>
            ...
            <div className="sidebar-card">
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
                ...
                {upcoming.length > 0 && (
                  <span style={{ marginLeft:"auto", fontSize:11, fontWeight:700, color:"#22D3EE", background:"rgba(34,211,238,0.1)", border:"1px solid rgba(34,211,238,0.2)", borderRadius:999, padding:"2px 8px" }}>{upcoming.length}</span>
                )}
              </div>
              ...
              {upcoming.length > 0 ? upcoming.map(ev => {
                ...
                <div style={{ textAlign:"center", padding:"24px 0", color:"#5A6386", fontSize:13 }}>
