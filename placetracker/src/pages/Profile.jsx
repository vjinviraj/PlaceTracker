  ...
  return (
    <div className={`profile-card${danger?" profile-card-danger":""}`}>
      <div className="profile-card-header">
        <div className="card-icon-wrap" style={{ background:danger?"rgba(239,68,68,0.12)":"rgba(34,211,238,0.1)", borderColor:danger?"rgba(239,68,68,0.25)":"rgba(34,211,238,0.2)" }}>
...
function PasswordInput({ value, onChange, show, onToggle, placeholder }) {
  return (
    <div style={{ position:"relative" }}>
      <input type={show?"text":"password"} value={value} onChange={onChange} placeholder={placeholder||"••••••••"} className="field-input" style={{ paddingRight:40 }}/>
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" style={{ maxWidth:400 }} onClick={(e)=>e.stopPropagation()}>
        ...
        <p style={{ fontSize:13.5, color:"#8FA0D7", lineHeight:1.75, marginBottom:20 }}>
          This will permanently delete your account and all your data. <strong style={{color:"#E5E9F7"}}>This cannot be undone.</strong>
        </p>
        <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:22 }}>
          <label className="field-label">Type your email to confirm: <span style={{color:"#22D3EE"}}>{email}</span></label>
          <input value={val} onChange={(e)=>setVal(e.target.value)} placeholder={email} className="field-input"
            style={{ borderColor: val && val !== email ? "rgba(239,68,68,0.4)" : undefined }}/>
        </div>
        <div style={{ display:"flex", gap:12 }}>
          <button className="btn-ghost" style={{flex:1}} onClick={()=>{ onClose(); setVal(""); }}>Cancel</button>
          <button className="btn-danger" style={{ flex:1, opacity:val!==email?0.5:1 }} onClick={()=>{ onConfirm(); setVal(""); }} disabled={val!==email}>Delete Account</button>
        </div>
      </div>
    </div>
  );
...
export default function Profile() {
  ...
  // Update name via backend → Clerk
  const handleUpdateName = async (e) => {
    e.preventDefault();
    if (!nameForm.firstName.trim()) { toast.error("First name cannot be empty"); return; }
    setSavingName(true);
    try {
  ...
  // Change password directly via Clerk (no backend needed)
  ...
  // Delete account via backend (clears DB data + Clerk account)
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#02050E", overflowX:"hidden", paddingTop:80, fontFamily:"Montserrat, sans-serif", position:"relative" }}>
      <style>{`
        ...
        .pf-grid { position:fixed; inset:0; pointer-events:none; z-index:0; background-image:linear-gradient(rgba(143,160,215,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(143,160,215,0.03) 1px,transparent 1px); background-size:50px 50px; }
        ...
        .btn-save:hover:not(:disabled) { transform:translateY(-2px); box-shadow:0 10px 24px rgba(34,211,238,0.4); }
        ...
        .btn-delete-row:hover { background:rgba(239,68,68,0.15); border-color:rgba(239,68,68,0.45); }
      ...
      <div style={{ maxWidth:680, margin:"0 auto", padding:"40px 24px", position:"relative", zIndex:2 }}>
        ...
        <div className="avatar-card" style={{ opacity:mounted?1:0, transform:mounted?"none":"translateY(14px)", transition:"opacity 0.6s ease 0.08s, transform 0.6s ease 0.08s" }}>
          <div className="avatar-circle">
            {user?.imageUrl ? <img src={user.imageUrl} alt={user.name}/> : initials}
          </div>
          <div>
            ...
            <form onSubmit={handleUpdateName} style={{ display:"flex", flexDirection:"column", gap:12 }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                  ...
                  <input value={nameForm.firstName} onChange={(e)=>setNameForm({...nameForm,firstName:e.target.value})} placeholder="First name" className="field-input"/>
                </div>
                <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                  <label className="field-label">Last Name</label>
                  <input value={nameForm.lastName} onChange={(e)=>setNameForm({...nameForm,lastName:e.target.value})} placeholder="Last name" className="field-input"/>
                </div>
              </div>
              <div><button type="submit" className="btn-save" disabled={savingName}><Save size={14}/> {savingName ? "Saving..." : "Save"}</button></div>
            </form>
          </SectionCard>
        </div>
        ...
        <div style={{ opacity:mounted?1:0, transform:mounted?"none":"translateY(14px)", transition:"opacity 0.6s ease 0.20s, transform 0.6s ease 0.20s" }}>
          <SectionCard icon={Lock} title="Change Password">
            <form onSubmit={handleUpdatePassword} style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div><button type="submit" className="btn-save" disabled={savingPass}><Save size={14}/> {savingPass ? "Updating..." : "Update Password"}</button></div>
            </form>
          </SectionCard>
        </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              ...
              <div className="danger-row danger-row-red">
                <div>
                  ...
                  <div style={{ fontSize:12, color:"#8FA0D7", marginTop:2 }}>Permanently delete your account and all data</div>
                </div>
